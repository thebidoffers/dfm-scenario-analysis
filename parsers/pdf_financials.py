...
def _regex_candidates(text: str, page_number: int, section: Optional[str]) -> List[Candidate]:
    candidates: List[Candidate] = []
    for metric, labels in {**INCOME_LABELS, **BALANCE_LABELS}.items():
        for label in labels:
            pattern = rf"{re.escape(label)}\s*(?:\d+\s*\(?[a-z]\)?\s*)?(?:[\s:]+)(-?\(?[\d,]+\)?)(?:\s+(-?\(?[\d,]+\)?))?"
...
def parse_pdf_financials(file) -> Dict[str, object]:
    ...
    with pdfplumber.open(file) as pdf:
        for page_number, page in enumerate(pdf.pages, start=1):
            ...
            candidates.extend(_regex_candidates(text, page_number, section))
