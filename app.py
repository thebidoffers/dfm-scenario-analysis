    ...
    try:
        with pdfplumber.open(file) as pdf:
            ...
            if m:
                ...
                data['items'].append(f"Investment Deposits: {fmt_smart(data['investment_deposits'])}")
            
            # Sukuks (Investments at amortised cost)
            m = re.search(r'Investments at amortised cost\s+\d+\s+([\d,]+)', text, re.I)
            if m:
                data['investments_amortised_cost'] = float(m.group(1).replace(',', ''))
                data['items'].append(f"Investments (Sukuks): {fmt_smart(data['investments_amortised_cost'])}")
